...
class DDPMPipeline_Img2Img(DDPMPipeline):
    ...
    def __call__(
        ...
        image1 = image.cpu().permute(0, 2, 3, 1).numpy()
        if output_type == "pil":
...
tmp_in = transform_cifar10(sample_image/ 2 + 0.5)
delta = torch.zeros_like(tmp_in)
delta.requires_grad = True
for _ in range(10):
    tmp_out = network_clf(tmp_in+delta)
    loss = F.cross_entropy(tmp_out, y_val)
    loss.backward()
    grad = delta.grad.detach()
    d = clamp(delta + alpha * grad.sign(), -epsilon, epsilon)
    d = clamp(d, lower_limit - tmp_in, upper_limit - tmp_in)
    delta.data = d
    delta.grad.zero_()

adv_out = ((tmp_in + delta)*std1+mu)
noisy_image = noise_schdeuler.add_noise((adv_out-0.5)*2, noise, timesteps)
plt.figure()
plt.imshow(((sample_image[0]/ 2 + 0.5).permute(1, 2, 0).cpu().detach().numpy() * 255.0).astype(np.uint8), vmin=0, vmax=255)
plt.show()

plt.figure()
plt.imshow((adv_out[0].permute(1, 2, 0).cpu().detach().numpy() * 255.0).astype(np.uint8), vmin=0, vmax=255)
plt.show()
# #
plt.figure()
